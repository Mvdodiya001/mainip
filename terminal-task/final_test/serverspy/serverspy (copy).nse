...
action = function(host, port)
	...
	if nmap.registry.args.serverspytestheadexisting ~= "0" then
		...
		if nmap.registry.args.serverspytestdeleteexisting ~= "0" then
		response = send_http_request(host, port, "DELETE", "/")
		if type(response) == "table" then
			identify_fingerprint(response, "scripts/serverspy/delete_existing/")
			save_response("scripts/serverspy/delete_existing.txt", response)
		else
			stdnse.print_debug(1, "serverspy: Failed to do delete_exisitng analysis")
			save_response("scripts/serverspy/delete_exisitng.txt", "Failed to fetch response")
		end
	end

	if nmap.registry.args.serverspytestoptions ~= "0" then
		response = send_http_request(host, port, "OPTIONS", "/")
		if type(response) == "table" then
			identify_fingerprint(response, "scripts/serverspy/options/")
			save_response("scripts/serverspy/options.txt", response)
		else
			stdnse.print_debug(1, "serverspy: Failed to do options analysis")
			save_response("scripts/serverspy/options.txt", "Failed to fetch response")
		end
	end

	if nmap.registry.args.serverspytestwrongmethod ~= "0" then
		response = send_http_request(host, port, "TEST", "/")
		if type(response) == "table" then
			identify_fingerprint(response, "scripts/serverspy/wrong_method/")
			save_response("scripts/serverspy/wrong_method.txt", response)
		else
			stdnse.print_debug(1, "serverspy: Failed to do wrong method analysis")
			save_response("scripts/serverspy/wrong_method.txt", "Failed to fetch response")
		end
	end

    	if nmap.registry.args.serverspytestwrongversion ~= "0" then
		response = send_http_request(host, port, "GET", "/", "9.8")
		if type(response) == "table" then
			identify_fingerprint(response, "scripts/serverspy/wrong_version/")
			save_response("wrong_version.txt", tostring(response))
		else
			stdnse.print_debug(1, "serverspy: Failed to do wrong version analysis")
			save_response("wrong_version.txt", "Failed to fetch response")
		end
	end
