...
const publish = (req, res, next) => {
	...
	let mosquittoPubsProcess = spawn(
		`echo ${process.env.ROOT_PASSWORD} | sudo bash publish.sh ${pubs.username} ${pubs.password} ${pubs.topic} ${pubs.ip} "${pubs.message}"`,
		{
			cwd: `${directory}/mqtt/codes/`,
			shell: true,
		}
	);
...
const subscribe = async (req, res, next) => {
	try {
		...
		let mosquittoSubsProcess = spawn(
			`echo ${process.env.ROOT_PASSWORD} | sudo -S bash ./subscribe.sh ${subs.username} ${subs.password} ${subs.topic} ${subs.ip}`,
			{
				cwd: `${directory}/mqtt/codes/`,
				shell: true,
			}
...
const bruteForce = (file, topic, username, directory) => {
	// Join the array elements with newline character '\n'

	const { io } = Constants;
	bruteForceProcess = spawn(
		`echo ${process.env.ROOT_PASSWORD} | sudo -S bash ./crackauth.sh ${file} ${topic} ${username}`,
		{
			cwd: `${directory}/mqtt/codes/crackauth`,
			shell: true,
		}
	);
...
const dosAttack = (req,res, next)=>{
	...
	let mosquittoDosProcess = spawn(
		`echo ${process.env.ROOT_PASSWORD} | sudo -S bash ./dos.sh ${dos.username} ${dos.password} ${dos.ip} ${dos.topic} ${dos.message}`,
		{
			cwd: `${directory}/mqtt/codes/`,
			shell: true,
		}
