...
const performDeauth = (
    ...
    const command = `./perform_deauth.sh ${interface} ${deviceMacAddress} ${accessPointMacAddress} ${timeout} ${ipAddress}`;
    console.log(command);
    return runSyncCommands(command);
};
...
const performDeauthantication = async (networkInterface, ipAddress, timeout) => {
    ...
    const response4 = prepareRespose(
        1,
        existingMonitorModeOutput.status,
